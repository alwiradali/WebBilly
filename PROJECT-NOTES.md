...
## Megacity: where a property comes from
10ninety is the only copy. Walid keeps his properties in his own system, marks
one **On The Market**, and it appears here; he takes it off the market and it
goes. His back office holds 88 records and nine were on the market on the first
sync — the rest are let rooms, deliberately not advertised.

- **The sync** (`worker/studio/tenninety-sync.js`) reads the feed and writes to
  D1. It refuses to act on a feed it could not read, never removes on an empty
  one, and stops if more than a third of the properties vanish at once. A
  removal sets `status='withdrawn'`; nothing is deleted.
- **The feed is not live.** 10ninety rebuilds it when the agent runs Portal
  Export, and again overnight. So the control is the **Refresh from 10ninety**
  button on the Studio's Listings screen, not a shorter cron.
- **`pinned`** (migration `0005_listings_pinned`) keeps a Studio-managed listing
  through a sync: the delete guard in `worker/studio/listings.js` refuses, and
  a pinned row cannot drag a sync over the removal threshold.

There were once fourteen hand-built pages in `templates/megacity-let-*.html`,
listed in `STATIC_LET_SLUGS`, served when D1 had no live row. They are gone.
Each one stated a rent, a deposit and an availability date typed by hand, and a
second copy of a rent that nobody is maintaining is a wrong rent waiting to
happen. `STATIC_LET_SLUGS` is now empty and stays as the honest answer to "is
this slug hand-built?". **`templates/megacity-let-template.html` is NOT one of
them** — `render.js` renders every live listing into it.

**No invented property facts**, and now that is enforced at the source rather
than on the page. `worker/studio/tenninety.js` leaves a field null rather than
guess: a `let_bond` of 0.0 is "not stated", not a £0 deposit; the EPC arrives as
a picture of the certificate so no rating is read off it; `let_furn_id` and
`let_type_id` are not mapped because their meanings were never confirmed; and an
HMO room does not inherit the house's bathroom count.
`node scripts/megacity-tenninety-check.mjs` asserts every one of those against
the feed 10ninety actually returned.
