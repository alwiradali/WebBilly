...
(function () {
  ...
  function showNotConnected() {
    var steps = [
      ["Enable R2 in the Cloudflare dashboard", "One click, once, in the account that will run this site: Cloudflare dashboard → R2 → Enable. A payment card must be on that account even for the free 10 GB tier — if the account is the client’s, that is their card, so ask before you get this far. Nothing else in the dashboard is needed.", []],
      ["Run the setup script", "From the repository on the main branch, on a machine where “npx wrangler whoami” shows the account named above — check it, this is the step that is hard to undo. It creates the bucket and the database, writes the real database id into wrangler.toml, applies the migration and sets the one-off setup token — and prints that token at the end. Keep it.", ["npx wrangler whoami", "bash scripts/megacity-setup.sh"]],
    ...
    authShell('<h1>Not connected yet</h1><p class="st-lead">The Studio needs its database and storage before anyone can sign in. Four steps, once. Doing it by hand instead (or on the Free plan) is described in <code>docs/megacity-studio.md</code>.</p>' +
      '<p class="st-hint"><b>First decide which Cloudflare account.</b> A Worker can only reach a database and storage in its own account, so the demo host and the client\u2019s own domain each need their own set. Running these steps here creates the client\u2019s listings, photos and 360 tours inside the agency\u2019s account \u2014 which then have to be built again and migrated across when the site moves to theirs. If the client\u2019s domain is where this is heading, do it once, there, and leave this unconnected. To show the Studio to someone meanwhile, add <code>?mock=1</code> to this address \u2014 no database needed.</p>' +
