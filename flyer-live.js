/* ============================================================
   Billy Digitals — Live 3D Flyer background
   A glowing brand-blue particle orb: ~18k points on a sphere,
   displaced by 3D simplex noise in the vertex shader, additive,
   through UnrealBloom. Gentle auto-rotate + pointer parallax.
   Self-contained Three.js. Degrades to nothing if WebGL is absent.
   ============================================================ */
...
(function () {
  "use strict";
  var canvas = document.getElementById("flyer-gl");
  if (!canvas) return;
  ...
  var vert = [
    ...
    "float n=snoise(p*0.32+vec3(0.0,uTime*0.14,0.0));",
    "float n2=snoise(p*0.75-uTime*0.10);",
    "vec3 dir=normalize(p);",
    "vec3 disp=p+dir*(n*1.1+n2*0.45);",
  ...
  var frag = [
    "precision highp float; uniform vec3 uCA; uniform vec3 uCB; uniform vec3 uCC;",
    "varying float vN; varying float vY;",
    "void main(){vec2 uv=gl_PointCoord-0.5;float d=length(uv);float a=smoothstep(0.5,0.02,d);if(a<=0.001)discard;",
    "vec3 col=mix(uCA,uCB,smoothstep(-1.0,1.0,vN));col=mix(col,uCC,smoothstep(-0.6,0.9,vY*0.16));",
    "gl_FragColor=vec4(col,a*0.92);}"
  ].join("\n");

  var uniforms = {
    uTime: { value: 0 }, uSize: { value: mobile ? 42 : 54 }, uPix: { value: renderer.getPixelRatio() },
  ...
  function loop(now) {
    ...
    group.rotation.y += dt * (reduce ? 0.02 : 0.06);
    group.rotation.x += (my * 0.22 - group.rotation.x) * 0.03;
