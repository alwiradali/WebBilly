# Billy Digitals — WhatsApp Business Catalogue

Ready-to-paste product list for the WhatsApp Business app. Each product has a
matching branded image in `assets/catalogue/`.

## How to add these in the app
**WhatsApp Business → Settings → Business tools → Catalogue → `+ Add item`**, then
for each product below: add the **image**, **name**, **price**, **description** and
**link** → **Save**.

Notes:
- Set the catalogue **currency to GBP (£)**.
- Web-design plans are **quote-based** — leave the price field blank (or set to 0
  and hide it) so each item reads "Free quote". Hosting and care are **per month**
  with fixed prices.

---

## Web design (one-off projects)

### 1. Standard Website — Free quote
- **Image:** `assets/catalogue/01-standard-website.png`
- **Link:** https://www.billydigitals.com/#contact
- **Description:**
  A professional website built from scratch to match your brand — up to 5 custom pages, fully responsive, with a contact form and WhatsApp built in. Essential on-page SEO, 2 revision rounds and 1 month of post-launch support. Tell us your project for a free, tailored quote — no hidden fees.

### 2. Premium Website — Free quote (Most popular)
- **Image:** `assets/catalogue/02-premium-website.png`
- **Link:** https://www.billydigitals.com/#contact
- **Description:**
  Our most popular build — up to 12 custom pages with a CMS so you can edit content yourself. Bespoke UI with premium animations, advanced SEO & speed, an AI chatbot, and managed hosting included. 4 revisions + 3 months support. Tell us your project for a free, tailored quote.

### 3. Ultra Premium Website — Free quote
- **Image:** `assets/catalogue/03-ultra-premium-website.png`
- **Link:** https://www.billydigitals.com/#contact
- **Description:**
  The full-service flagship — we become your complete digital department. Unlimited pages or a custom web app, complete branding & design system, AI automation, professional copywriting, 12 months of hosting & care, a dedicated project manager and unlimited revisions. Tell us your project for a free, tailored quote.

---

## Managed hosting (per month)

### 4. Starter Hosting — £5/month
- **Image:** `assets/catalogue/04-starter-hosting.png`
- **Link:** https://www.billydigitals.com/#hosting
- **Description:**
  Fast, secure UK hosting for personal sites and landing pages. Free SSL, daily backups, malware scanning and email support. £5/month.

### 5. Business Hosting — £10/month (Most popular)
- **Image:** `assets/catalogue/05-business-hosting.png`
- **Link:** https://www.billydigitals.com/#hosting
- **Description:**
  For growing businesses that can't afford downtime. Everything in Starter plus extra speed & storage, 24/7 uptime monitoring, priority fixes, business email accounts and a monthly health report. £10/month.

### 6. Premium Hosting — £20/month
- **Image:** `assets/catalogue/06-premium-hosting.png`
- **Link:** https://www.billydigitals.com/#hosting
- **Description:**
  Maximum firepower — the fastest UK server resources, advanced security hardening, daily backups, 24/7 monitoring and same-day support. Included free with Premium & Ultra Premium builds. £20/month.

---

## SEO & Growth Care Plans (per month)

### 7. Starter Care — £60/month
- **Image:** `assets/catalogue/07-starter-care.png`
- **Link:** https://www.billydigitals.com/#care
- **Description:**
  Keep growing while we keep watch. Monthly SEO health check, Google Business Profile monitoring, up to 2 content updates/month, broken-link & speed checks, and a monthly performance report. £60/month.

### 8. Growth Care — £70/month (Most popular)
- **Image:** `assets/catalogue/08-growth-care.png`
- **Link:** https://www.billydigitals.com/#care
- **Description:**
  For businesses ready to actively climb the rankings. Everything in Starter Care plus Google Business management & posts, keyword & ranking tracking, up to 5 content updates/month and priority support. £70/month.

### 9. Complete Care — £80/month
- **Image:** `assets/catalogue/09-complete-care.png`
- **Link:** https://www.billydigitals.com/#care
- **Description:**
  Your full-service SEO & growth department. Everything in Growth Care plus full on-page SEO, competitor tracking, unlimited minor content updates, a monthly strategy call and reputation management. £80/month.

---

## Optional: add-ons (mention in descriptions, or add as extra items)
AI chatbot · booking calendar & appointments · address/postcode finder · customer
login & register · online payments · Google reviews widget · WhatsApp live chat ·
photo galleries · blog & news · newsletter signup · multi-language · analytics
dashboard. Bolt onto any plan — link: https://www.billydigitals.com/#hosting
